  ...
  <GenericModal @close="$emit('close')" title="Your boards" button-id="sortButton" :top="160" :left="192">
    <div class="relative flex flex-col">
      ...
      <div v-if="showMenu" class="absolute flex flex-col w-[282px] pl-[1px] py-2 left-3 -bottom-[74px] bg-white dark:bg-dark shadow-custom rounded">
        <span @click="selectOption('Sort alphabetically')" :class="dataStore.sortingOption === 'Sort alphabetically' ? 'text-[#0C66E4] dark:text-[#579DFF] bg-[#E9F2FF] hover:bg-[#bcd0ec] dark:hover:bg-[#121a31] dark:bg-[#1C2B41] left-shadow' : 'hover:bg-[#0000001a] dark:hover:bg-[#ffffff15] left-shadow-hover'" class="h-8 px-3 flex items-center hover:cursor-pointer">
          Sort alphabetically
        </span>
        <span @click="selectOption('Sort by most recent')" :class="dataStore.sortingOption === 'Sort by most recent' ? 'text-[#0C66E4] dark:text-[#579DFF] bg-[#E9F2FF] hover:bg-[#bcd0ec] dark:hover:bg-[#121a31] dark:bg-[#1C2B41] left-shadow' : 'hover:bg-[#0000001a] dark:hover:bg-[#ffffff15] left-shadow-hover'"  class="h-8 px-3 flex items-center hover:cursor-pointer">
          Sort by most recent
        </span>
      </div>
    </div>
...
const selectOption = (option: string) => {
  dataStore.sortingOption = option
  emit('close')
}
</script>
